...
def parse_coord_str(coord_str: str) -> tuple[int, int, int] | None:
...
def main() -> None:
    ...
    try:
        if len(sys.argv) > 1:
            if len(sys.argv) > 2:
                ...
                coord_str: str = sys.argv[1]
                pos1 = parse_coord_str(coord_str)
                if pos1 is None:
                    return
        else:
            pos1 = (10, 20, 5)
    ...
    pos2 = parse_coord_str(coord_str_valid)
    ...
    parse_coord_str(coord_str_invalid)
    print()
